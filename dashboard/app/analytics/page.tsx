  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-zinc-500 text-sm mt-1">{data?.total || 0} total incidents analyzed</p>
      </div>
      <div className="glass rounded-2xl p-5">
        <h3 className="text-sm font-semibold text-white mb-4">Incident Types</h3>
          ...
          {Object.entries(byType).sort(([, a]: [string, any], [, b]: [string, any]) => b - a).map(([type, count]: [string, any]) => (
            <div key={type} className="flex items-center gap-3">
              ...
              <div className="flex-1 h-6 bg-white/5 rounded-full overflow-hidden">
                <div className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-violet-500 transition-all duration-700" style={{ width: `${(count / maxType) * 100}%` }} />
              </div>
              <span className="text-xs font-mono text-zinc-500 w-8 text-right">{count}</span>
            </div>
          ))}
          {Object.keys(byType).length === 0 && <p className="text-zinc-600 text-sm">No data yet. Submit incidents from the Validate page.</p>}
        </div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="glass rounded-2xl p-5">
          <h3 className="text-sm font-semibold text-white mb-4">By Sector</h3>
          <div className="space-y-2">
            ...
            {Object.keys(bySector).length === 0 && <p className="text-zinc-600 text-sm">No sector data yet.</p>}
          </div>
        </div>
        <div className="glass rounded-2xl p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            {Object.entries(bySeverity).map(([sev, count]: [string, any]) => {
              const colors: Record<string, string> = { critical: "text-rose-400 bg-rose-400/10", high: "text-amber-400 bg-amber-400/10", medium: "text-violet-400 bg-violet-400/10", low: "text-emerald-400 bg-emerald-400/10" };
              return (
                <div key={sev} className={`rounded-xl p-4 text-center ${colors[sev] || "text-zinc-400 bg-white/5"}`}>
                  <p className="text-xs font-mono uppercase opacity-70">{sev}</p>
